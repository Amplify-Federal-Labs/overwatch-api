...
describe("CRON_JOBS", () => {
	it("has three jobs in order: fpds, rss, enrichment", () => {
...
describe("getScheduledJob", () => {
	...
	it("cycles back to fpds at hour 3", () => {
		expect(getScheduledJob(3).name).toBe("fpds");
	});

	it("cycles through all 24 hours correctly", () => {
		const expected = ["fpds", "rss", "enrichment"];
		for (let hour = 0; hour < 24; hour++) {
			expect(getScheduledJob(hour).name).toBe(expected[hour % 3]);
		}
	});
});
